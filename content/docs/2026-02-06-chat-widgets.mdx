...
The approach that worked—and the one we stuck with—was to remove the weather tool entirely.

We realized that having instructions for both "how to check the weather" and "how to use a weather widget" was redundant and added extra steps.

The bottleneck was the agent having to call a weather tool and then pass the data as expensive, slow output tokens into an XML tag.

The fewer output tokens (again, slow and expensive), the better. So we gave it a simple widget that required very few tokens to call, and moved our forecast-fetching logic into an API endpoint that could respond with JSON fast.
